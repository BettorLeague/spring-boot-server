...
public class SwaggerConfig {
    ...
    public Docket api() {
        return new Docket(DocumentationType.SWAGGER_2)
                ...
                .paths(PathSelectors.regex("(?!/signin).+"))
                .paths(PathSelectors.regex("(?!/auditevents).+"))
                .paths(PathSelectors.regex("(?!/autoconfig).+"))
                .paths(PathSelectors.regex("(?!/metrics).+"))
                .paths(PathSelectors.regex("(?!/loggers).+"))
                .paths(PathSelectors.regex("(?!/heapdump).+"))
                .paths(PathSelectors.regex("(?!/health).+"))
                .paths(PathSelectors.regex("(?!/env).+"))
                .paths(PathSelectors.regex("(?!/beans).+"))
                .paths(PathSelectors.regex("(?!/configprops).+"))
                .paths(PathSelectors.regex("(?!/dump).+"))
                .paths(PathSelectors.regex("(?!/heapdump).+"))
